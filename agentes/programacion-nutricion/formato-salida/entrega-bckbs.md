...
**Los resultados de FatSecret vienen en inglés** (plan Basic de la cuenta, sin `region` propia). **ACTUALIZADO 2026-09-20: FatSecret ya dio permiso explícito para traducir también el contenido de receta** (nombre + `directions`), no solo nombres de ingrediente suelto como decía esta sección hasta ahora -- pero **la traducción todavía no está implementada** (bloqueada por falta de credenciales de un servicio de traducción, ver `Bckbs::docs/FATSECRET_INTEGRATION.md` sección 10 para el diseño completo pendiente). Hasta que esa sección deje de decir "pendiente": el Productor sigue viendo/documentando el contenido en inglés tal cual lo devuelve la API, no debe traducirlo él mismo por su cuenta -- la traducción, cuando exista, vivirá en el backend (Bckbs) y llegará ya traducida en los mismos campos `name`/`directions`, sin que el Productor tenga que hacer nada distinto.
