...
**Los resultados de FatSecret vienen en inglés** (plan Basic de la cuenta, sin `region` propia). Documenta esto en el razonamiento igual que cualquier otra decisión, pero **NO traduzcas el nombre/descripción de la receta** -- el permiso de traducción que se le pidió a FatSecret cubre solo nombres de ingredientes sueltos (ver `Bckbs::docs/FATSECRET_INTEGRATION.md` sección 0), no su contenido de receta. El cliente verá el título en inglés en la app tal cual -- es una limitación conocida y aceptada, no algo que el Productor deba intentar arreglar por su cuenta traduciendo.
