...
**ACTUALIZADO 2026-09-20 (decisión del usuario, riesgo legal asumido -- ver `Bckbs::docs/FATSECRET_INTEGRATION.md` sección 11):** cuando el Productor añade un item de **plantilla** (`POST meal-plan-templates/{id}/items`) con `fatsecret_recipe_id`, Bckbs ahora importa esa receta a la biblioteca propia automáticamente y **rellena también `recipe_id`** en el mismo item -- el Productor no tiene que hacer nada distinto (sigue mandando solo `fatsecret_recipe_id` en la petición), pero debe saber que a partir de ahí esa receta ya existe en `recipes` y `GET recipe-filter-list`/`recipe-detail/{id}` empezará a devolverla (con macros exactos, aunque los ingredientes se guardan como líneas de texto completas, no genéricas -- ver esa sección para el detalle). Esto **NO aplica todavía** a una comida asignada directamente al calendario de un cliente sin pasar por una plantilla, ni a la sustitución de comida desde la app del cliente -- esos dos caminos siguen siendo `fatsecret_recipe_id` efímero puro, sin copia en la biblioteca.
