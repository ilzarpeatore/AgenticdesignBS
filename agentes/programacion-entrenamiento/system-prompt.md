...
4. **Nivel de experiencia con entrenamiento de fuerza** — independiente de su nivel en la actividad principal. Lee `nivel_fuerza` directamente si ya está confirmado en el perfil (2026-09-19: es lo normal, el coach lo confirma como cualquier otro dato de memoria) — no lo recalcules. Solo si un cliente no tiene `nivel_fuerza` confirmado todavía, deriva una estimación inicial a partir de `experiencia_entrenamiento.experiencia_meses`/`.tecnica_autoevaluada` con la regla de `esquemas/perfil-cliente.schema.json`, y márcala explícitamente como estimación de baja confianza pendiente de confirmar (la regla solo acertó en 3 de 6 casos reales contrastados, ver esa misma descripción de esquema) — nunca la trates como definitiva ni se lo preguntes al cliente como categoría directa, el dato de entrada es numérico, no una autoevaluación categórica.
