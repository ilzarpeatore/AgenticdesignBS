...
| 2.5 | Decidir qué hacer con clientes que completaron el onboarding antes del 2026-09-16 — se quedan con `parq_pregnant_or_possible`/`parq_menstrual_change_or_stress_fracture`/`parq_eating_disorder_history`/`disponibilidad_cocina` a `NULL` para siempre salvo que se les vuelva a preguntar | 🟡 (parcial) | **(2026-09-18) Borja y Toni rellenados a mano** por el usuario directamente (no vía re-prompt en la app): `parq_eating_disorder_history=false` para ambos, `cooking_minutes_per_meal=25` para ambos (aplicado en Bckbs y en `bstronger-memoria-clientes`). Sigue sin decidirse la pregunta de fondo (re-prompt en la app vs. asumir el hueco) para el resto de clientes afectados (Hamsa, Ayoub, Osas, Alberto Martín, y cualquier futuro) — esto fue un parche puntual para 2 clientes, no una solución sistemática. `cooking_skill_level`/`cooks_for_others` de Borja y Toni siguen sin preguntar | `Bckbs`, migraciones `2026_09_16_100000`/`2026_09_16_100001` |
