...
>
> **Última actualización:** 2026-09-17 (sesiones en paralelo con acceso real a `bestronger-vps`: 1.1 cerrada del todo — `--confidence-gate` probado con casos ambiguos reales contra un catálogo nuevo y norma de asignación manual establecida; 1.2 confirmada ya aplicada en producción; 2.3 parcial — Borja resuelto, Toni sigue abierto a propósito; 2.6 resuelto — repo privado `ilzarpeatore/bstronger-memoria-clientes` creado; 2.7 resuelto — módulo `habitos-prioritarios.md` en el agente de nutrición; nuevo ítem 1.4 — volcado inicial de perfiles reales antes de generar nada, instrucciones en `VOLCAR_PERFILES_ONBOARDING.md`)
...
| 1.4 | Volcado inicial: crear `perfil-cliente.json`/`perfil-nutricional.json` en `bstronger-memoria-clientes` para todos los clientes con onboarding real completo, antes de generar ninguna rutina/plan | 🟡 | **(2026-09-17)** Decisión del usuario: primero memoria completa de todos los clientes, luego generación. Mapeo completo verificado contra el código real de Bckbs (modelos/migraciones) en `VOLCAR_PERFILES_ONBOARDING.md` (raíz de este repo) — falta ejecutarlo desde una sesión con acceso VPS real (solo lectura contra Bckbs, escritura solo en `bstronger-memoria-clientes`). Deja explícitamente pendientes de revisión del coach: cribado con algún "sí" (no se autoclasifica relativa/absoluta), alergias en texto libre sin severidad, y todo lo que no tiene fuente en el onboarding (`contexto_vida`, `actividad_principal`, etc. — eso lo completa después el archivo manual que el usuario entrega por cliente) | `VOLCAR_PERFILES_ONBOARDING.md` |
