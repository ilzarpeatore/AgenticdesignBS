...
>
> **Última actualización:** 2026-09-17 (sesión con acceso real a `bestronger-vps`: 1.1 cerrada del todo — `--confidence-gate` probado con casos ambiguos reales y norma de asignación manual establecida; 1.2 confirmada ya aplicada en producción; más el diseño de memoria persistente por cliente, sección 2.6/2.7)
...
| 2.3 | Migrar a `client_limitations` las alergias que hoy solo existen como texto libre en `nutrition_questionnaire_answers.allergies_intolerances` | 🟡 | **(2026-09-17) Listado ya generado** contra la BD real (consulta de solo lectura, sección 5 del BRIEF) — 4 filas con texto no vacío/no "ninguna", ninguna tiene fila estructurada en `client_limitations` todavía: `user_id 8` (demo, texto "B", ignorable) · `user_id 99` Toni Pérez Fernández, "fruta/lactosa" · `user_id 100` (usuario de prueba, texto "Bbbbb", ignorable) · `user_id 101` Borja Betanzos, "Tengo alergia a la mayoría de pieles de frutas, pero como igual fruta, no es una alergia preocupante". Falta la parte humana: que el coach decida `type`/`severity` para Toni y Borja y cree las filas vía el endpoint ya existente (`POST client-limitation-store`) — a propósito no se automatiza el parseo | `BRIEF_registro_alergias_intolerancias.md`, sección 5 |
...
| 2.6 | Decidir dónde viven físicamente los archivos reales de memoria por cliente (`log-registro`/`log-nutricion`/`checkpoint-fisico`) — el diseño (2026-09-17) define el formato pero deja explícitamente que la ubicación real la decida quien opera el agente, no este repositorio (son datos sensibles de clientes) | 🟡 | Decisión operativa del usuario: ¿carpeta local privada, en algún momento una tabla en Bckbs? No bloquea el diseño, pero sin esto ningún agente puede de verdad leer/escribir memoria todavía | `agentes/programacion-entrenamiento/system-prompt.md`, sección "Memoria del cliente" |
| 2.7 | ¿Añadir una capa de "hábitos prioritarios" (nutrición/estilo de vida, ordenados por impacto) a alguno de los dos agentes? Aparece en el caso real de Borja (guideline compartido 2026-09-17) pero ningún agente genera este tipo de contenido hoy | 🟡 | Decisión de alcance — es contenido nuevo, no un ajuste de lo existente; no construir sin que el usuario decida si lo quiere y en qué agente encaja | `agentes/programacion-entrenamiento/CHANGELOG.md`, v0.15.0 |
