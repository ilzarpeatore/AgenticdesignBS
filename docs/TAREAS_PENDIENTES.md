...
>
> **Última actualización:** 2026-09-18 (1.1 y 1.2 cerradas del todo; 1.4 resuelto — volcado inicial de 6 perfiles reales a `bstronger-memoria-clientes`, ampliado después con 3 guidelines de coach reales (Hamsa, Toni, Borja); 2.3 cerrado del todo — Toni confirmó lactosa como intolerancia/leve, ambas restricciones suyas ya en `client_limitations` de Bckbs; 2.5 parcial — Borja/Toni rellenados a mano, sigue sin decisión sistemática para el resto; 2.6 y 2.7 resueltos)
...
| 2.5 | Decidir qué hacer con clientes que completaron el onboarding antes del 2026-09-16 — se quedan con `parq_pregnant_or_possible`/`parq_menstrual_change_or_stress_fracture`/`parq_eating_disorder_history`/`disponibilidad_cocina` a `NULL` para siempre salvo que se les vuelva a preguntar | 🟡 (parcial) | **(2026-09-18) Borja y Toni, completo del todo.** El usuario los rellenó a mano directamente (no vía re-prompt en la app): `parq_eating_disorder_history=false` para ambos; `cooking_minutes_per_meal=25`, `cooking_skill_level=beginner` para ambos; `cooks_for_others=false` (Toni) / `true` (Borja). Aplicado en Bckbs y en `bstronger-memoria-clientes` — sin ningún `_pendiente_revision_coach` de nutrición para ninguno de los dos. Sigue sin decidirse la pregunta de fondo (re-prompt en la app vs. asumir el hueco) para el resto de clientes afectados (Hamsa, Ayoub, Osas, Alberto Martín, y cualquier futuro) — esto fue un parche puntual para 2 clientes, no una solución sistemática | `Bckbs`, migraciones `2026_09_16_100000`/`2026_09_16_100001` |
