...
| Agente Importador de Programas | Lleva el `.xlsx` ya generado hasta la base de datos real (importar, verificar integridad) — no genera contenido, no decide programación, y **nunca asigna a un cliente real** (decisión del usuario, 2026-09-17): eso es siempre manual, en el panel admin | `ilzarpeatore/Bckbs` (Laravel, VPS `bestronger-vps`) | `agentes/importador-programas/system-prompt.md` |
