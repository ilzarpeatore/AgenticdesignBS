...
- **(2026-09-15) Asistente de Programación de Nutrición** — tiene ya capa de seguridad con deep search clínico real, cinco módulos de contenido (energía/macros, timing, recomposición, superávit de ganancia muscular, rendimiento deportivo/resistencia), recetario y formato de salida real (la API de Bckbs ya existía, ver `formato-salida/entrega-bckbs.md`), esquema de log y validador determinista (`validador/validar_plan.py`, con fixtures sintéticos). Pendiente real: (1) registro estructurado de severidad de alergias en Bckbs — encargo aparte, `BRIEF_registro_alergias_intolerancias.md`, entregado al usuario, sin ejecutar todavía; (2) etiquetado de alérgenos por ingrediente en Bckbs (`ingredients` no tiene esa columna) — depende de (1); (3) poblaciones específicas (embarazo, patologías) — explícitamente pospuesto, solo si aparece un cliente real que lo necesite; (4) un caso real de principio a fin (como el de Toni) antes de dar por maduro el diseño — candidato natural: uno de los recetarios por cliente que el usuario ya tiene hechos a mano.
